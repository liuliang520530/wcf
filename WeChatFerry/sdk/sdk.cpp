...
int WxInitSDKWithPid(DWORD pid, bool debug, int port)
{
  ...
  if (pid == 0)
  {
    MessageBox(NULL, L"无效的进程ID", L"WxInitSDKWithPid", 0);
    return -1;
  }
  ...
  wcProcess = inject_dll(pid, spyDllPath.c_str(), &spyBase);
  if (wcProcess == NULL)
  {
    MessageBox(NULL, L"注入失败", L"WxInitSDKWithPid", 0);
    return -1;
  }
...
int WxInitSDKWithPath(const wchar_t *wxPath, bool debug, int port)
{
  ...
  {
    MessageBox(NULL, L"启动微信失败", L"WxInitSDKWithPath", 0);
    return GetLastError();
  }
  CloseHandle(pi.hThread);

  std::this_thread::sleep_for(std::chrono::seconds(2));
  wcProcess = inject_dll(pi.dwProcessId, spyDllPath.c_str(), &spyBase);
  if (wcProcess == NULL)
  {
    MessageBox(NULL, L"注入失败", L"WxInitSDKWithPath", 0);
